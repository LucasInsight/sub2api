...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready XTrust-AI setups.
  ...
  const configContent = `# Grok Build CLI → XTrust-AI Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for XTrust-AI Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches XTrust-AI).
...
description = "Grok 4.5 via XTrust-AI (Responses)"
...
# Imagine tools: model IDs go to XTrust-AI media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → XTrust-AI Grok group
...
# Grok/XTrust-AI path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at XTrust-AI OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via XTrust-AI'
