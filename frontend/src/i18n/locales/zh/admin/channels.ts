export default {
    ...
    subscriptions: {
      ...
      upgrading: '升级中...',
      upgradeSource: '当前订阅',
      upgradeTarget: '目标订阅分组',
      upgradeUsage: '将继承的当前用量',
      selectUpgradeTarget: '选择目标订阅分组',
      noUpgradeTargets: '没有可升级的同平台订阅分组',
      upgradeWarning: '升级后将继承当前用量、用量窗口和到期时间，源订阅会被撤销，绑定源分组的 API Key 会迁移到目标分组。',
      restore: '恢复',
      resetQuota: '重置配额',
      resetQuotaTitle: '重置用量配额',
      resetQuotaConfirm: "确定要重置 '{user}' 的 5 小时、每日、每周和每月用量配额吗？用量将归零并重新计算。",
      quotaResetSuccess: '配额重置成功',
      failedToResetQuota: '重置配额失败',
      resetAllQuota: '重置全部配额',
      resetAllQuotaTitle: '重置全部订阅配额',
      resetAllQuotaConfirm: '将重置全部 {count} 个有效订阅的 5 小时、每日、每周和每月配额。此操作不可撤销。',
      resetAllQuotaAcknowledgement: '我已确认 OpenAI 官方额度已重置',
      resetAllQuotaSuccess: '已重置 {count} 个有效订阅的配额',
      failedToResetAllQuota: '重置全部订阅配额失败',
      checkingResetAllQuota: '正在检查全量重置状态',
      resettingAllQuota: '正在重置全部订阅配额',
      resetAllQuotaLocked: '当前不可重置全部配额',
      resetAllQuotaNoActive: '当前没有可重置的有效订阅',
      resetAllQuotaReady: '可重置 {count} 个有效订阅',
      ...
      clearFalsePositiveTitle: '清理提前重置误报',
      clearFalsePositiveConfirm: '仅清除所选账号的待处理标记，不会重置订阅配额；已有审计日志将继续保留。',
      clearFalsePositiveAccount: '待处理事件',
      clearFalsePositiveOption: '{account} · 探测于 {time}',
      clearFalsePositiveDescription: '清除指定账号的误报待处理标记，不重置订阅配额',
      clearFalsePositiveNoPending: '当前没有待清理的提前重置事件',
      clearingFalsePositive: '正在清理误报标记',
      clearFalsePositiveSuccess: '已清除账号 {account} 的误报标记',
      failedToClearFalsePositive: '清理误报标记失败，请刷新后重试',
      ...
      failedToUpgrade: '升级订阅失败',
      failedToLoadUpgradeTargets: '加载可升级分组失败',
      errors: {
        SUBSCRIPTION_UPGRADE_SOURCE_INVALID: '源订阅必须处于生效且未过期状态',
        SUBSCRIPTION_UPGRADE_SAME_GROUP: '目标分组不能与当前分组相同',
        SUBSCRIPTION_UPGRADE_TARGET_INACTIVE: '目标分组未启用',
        SUBSCRIPTION_UPGRADE_PLATFORM_MISMATCH: '目标分组必须与当前分组属于同一平台',
        SUBSCRIPTION_UPGRADE_TARGET_EXISTS: '用户已经拥有目标分组的订阅',
        GROUP_NOT_SUBSCRIPTION_TYPE: '目标分组不是订阅类型'
      },
      ...
      guide: {
        title: '订阅管理教程',
        subtitle: '订阅模式允许你按时间周期为用户分配使用额度，支持 5 小时/日/周/月配额限制。按照以下步骤即可完成配置。',
        showGuide: '使用指南',
        step1: {
          title: '创建订阅分组',
          line1: '前往「分组管理」页面，点击「创建分组」',
          line2: '将计费类型设为「订阅」，配置 5 小时/日/周/月额度限制',
          line3: '保存分组，确保状态为「正常」',
          link: '前往分组管理'
        },
        step2: {
        ...
        actions: {
          ...
          resetQuota: '重置配额',
          resetQuotaDesc: '将 5 小时/日/周/月用量归零，重新开始计算',
          revoke: '撤销',
