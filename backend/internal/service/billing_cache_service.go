...
// 注：ErrFiveHourLimitExceeded/ErrDailyLimitExceeded/ErrWeeklyLimitExceeded/ErrMonthlyLimitExceeded在subscription_service.go中定义
var (
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID int64, group *Group, subscription *UserSubscription) error {
	...
	if subscription != nil {
		if subscription.NeedsFiveHourReset() {
			fiveHourUsage = 0
		}
	...
	// 检查限额（使用传入的Group限额配置）
	if group.HasFiveHourLimit() && fiveHourUsage >= *group.FiveHourLimitUSD {
