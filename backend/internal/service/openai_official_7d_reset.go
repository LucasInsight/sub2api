...
func observeOpenAIOfficial7dReset(
	...
	if observer == nil || observer.repository == nil || accountID <= 0 || observedAt.IsZero() || resetAt.IsZero() {
		return false, nil
	}
...
func openAIOfficial7dResetTimesFromExtraUpdates(
...
) (observedAt, resetAt time.Time, ok bool) {
	if len(updates) == 0 {
	...
	return observedAt.UTC(), resetAt.UTC(), true
}
